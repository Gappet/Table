...
namespace ASTImpl {
    ...
    public:
        ...
        virtual double Evaluate(std::function <CellInterface::Value(const Position& pos)> func) const = 0;
    ...
    namespace {
        ...
        public:
            ...
            double Evaluate(std::function <CellInterface::Value(const Position& pos)> func) const override {
                auto type = this->GetPrecedence();
                double out;
                if (type == ExprPrecedence::EP_ADD) {
                    out = lhs_.get()->Evaluate(func) + rhs_.get()->Evaluate(func);
                }
                else if (type == ExprPrecedence::EP_MUL) {
                    out = lhs_.get()->Evaluate(func) * rhs_.get()->Evaluate(func);
                }
                else if (type == ExprPrecedence::EP_SUB) {
                    out = lhs_.get()->Evaluate(func) - rhs_.get()->Evaluate(func);
                }
                else if (type == ExprPrecedence::EP_DIV) {
                    out = lhs_.get()->Evaluate(func) / rhs_.get()->Evaluate(func);
                    if (!std::isfinite(out)) {
                        FormulaError::Category a = FormulaError::Category::Div0;
                        throw FormulaError(a);
                    }
                }
        ...
        public:
            ...
            double Evaluate(std::function <CellInterface::Value(const Position& pos)> func) const override {
                if (type_ == Type::UnaryMinus) {
                    return operand_.get()->Evaluate(func) * -1;
                }
                return operand_.get()->Evaluate(func);
            }
        ...
        public:
            ...
            double Evaluate(std::function <CellInterface::Value(const Position& pos)> func) const override {
                ...
                try {
                    ...
                    if (std::get_if<FormulaError>(&out)) {
                        throw std::get<FormulaError>(out);
                    }
                }
...
double FormulaAST::Execute(std::function <CellInterface::Value(const Position& pos)> func) const {
